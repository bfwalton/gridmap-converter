...
func convert(gridmap: GridMap, use_multimesh: bool, include_collision: bool = true) -> Node3D:
	...
	if use_multimesh:
		_convert_to_multimesh(gridmap, used_cells, mesh_lib, parent, include_collision)
	else:
		_convert_to_mesh_instances(gridmap, used_cells, mesh_lib, parent, include_collision)
...
func _convert_to_multimesh(gridmap: GridMap, used_cells: Array, mesh_lib: MeshLibrary, parent: Node3D, include_collision: bool) -> void:
...
func _convert_to_mesh_instances(gridmap: GridMap, used_cells: Array, mesh_lib: MeshLibrary, parent: Node3D, include_collision: bool) -> void:
	for cell in used_cells:
		var mesh_id = gridmap.get_cell_item(cell)
		if mesh_id == -1:
			continue
